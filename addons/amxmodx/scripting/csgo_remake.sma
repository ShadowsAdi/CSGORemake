...
public plugin_init()
{
	...
	new pcvar = create_cvar("csgor_author", "Shadows Adi", FCVAR_SERVER|FCVAR_SPONLY, "Plugin's author." )
...
public _SaveData(id)
{
	...
	switch(g_iCvars[iSaveType])
	{
		...
		case MYSQL:
		{
			...
			server_print("timestamp: %d", iTimestamp)
...
public _ShowBonusMenu(id)
{
	...
	switch(g_iCvars[iSaveType])
	{
		...
		case MYSQL:
		{
			new Handle:iQuery = SQL_PrepareQuery(g_iSqlConnection, "SELECT * FROM `csgor_data` WHERE `%s` = ^"%s^";", iNum == 0 ? "Last IP" : "SteamID", g_szUserLastIP[id])
...
IsTaken(id, &iTimestamp)
{
	...
	switch(g_iCvars[iSaveType])
	{
		...
		case MYSQL:
		{
			...
			server_print("time: %d", iTimestamp)
